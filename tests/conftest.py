...
async def clean_db(db_session: AsyncSession) -> AsyncGenerator[AsyncSession, None]:
    """Provide a clean database for each test (truncates all tables)."""
    # Truncate all tables
    await db_session.execute(text("TRUNCATE repositories, reviews, review_comments, prompt_versions CASCADE"))
